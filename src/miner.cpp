...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("STEMMAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("STEMMAMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("STEMMAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stemma-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("STEMMAMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running STEMMAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadSTEMMAMiner() exception");
    } catch (...) {
        LogPrintf("ThreadSTEMMAMiner() exception");
    }

    LogPrintf("ThreadSTEMMAMiner exiting\n");
}
